    ...
    public class PlayerLook : MonoBehaviour
    {
        ...
        public void ProcessLook(Vector2 input)
        {
            if (!isLookingInputLocked)
            {
                ...
                xRotation -= (mouseY / 80f) * MouseSensivity.YSensitivity;
                xRotation = Mathf.Clamp(xRotation, -80f, 70f);
                cam.transform.localRotation = Quaternion.Euler(xRotation, 0, 0);
                float yRotation = (mouseX / 80f) * MouseSensivity.XSensitivity;
                PlayerCameraCurRotation = new Vector3(xRotation, yRotation, 0);
                transform.Rotate(Vector3.up * (mouseX / 80f) * MouseSensivity.XSensitivity);
            }
        }
    }
}